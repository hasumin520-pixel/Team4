...
scripts/
  ...
  data-src/             # 엑셀에서 추출한 sheet1.xml, sharedStrings.xml
src/
  app/                  # layout(모바일 컬럼), page
  components/
    MoimApp.tsx         # 필터(목적/예산/거리/음식종류)·정렬·뷰 전환 컨테이너
    RestaurantCard.tsx  # 목록 카드 (방문횟수 랭킹)
    MapView.tsx         # SVG 목업 지도 (거리 동심원 + 핀, 핀 크기=방문횟수)
    DetailSheet.tsx     # 상세 바텀시트 (최근 카드 사용 내역, 예약 버튼)
  data/
    restaurants.json    # 더미 식당 36곳 (주변 실존 식당명 + 가상 평점/좌표)
    visits.json         # 생성물 — 식당별 방문 통계
  lib/data.ts           # 타입, 파생 필드(가중평균 평점 등), 상수
```
...
- 식당명은 실존 상호지만 평점·리뷰수·거리·좌표는 가상 값
